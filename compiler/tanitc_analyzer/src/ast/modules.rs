use tanitc_ast::{ModuleDef, VisitorMut};
...
impl Analyzer {
    pub fn analyze_module_def(&mut self, module_def: &mut ModuleDef) -> Result<(), Message> {
        ...
        analyzer.prefix = self.prefix.clone();
        analyzer.prefix.push(module_def.identifier);

        if let Some(body) = &mut module_def.body {
            if let Err(err) = analyzer.visit_block(body) {
                self.error(err);
            }
