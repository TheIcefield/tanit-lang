...
impl CodeGenStream<'_> {
    pub fn generate_control_flow(&mut self, cf: &ControlFlow) -> std::io::Result<()> {
        use std::io::Write;
...
mod tests {
    ...
    fn get_integer(value: usize) -> Expression {
        Expression::Literal(literal::Literal::Integer(literal::Integer {
            location: Location::default(),
            value,
        }))
    }

    fn get_return(ret: Option<Box<Expression>>) -> ControlFlow {
        ControlFlow {
            location: Location::default(),
        ...
        let node = Hir::from(Block {
            ...
            statements: vec![Block {
                ...
                statements: vec![
                    ...
                    get_return(Some(Box::new(get_integer(0)))).into(),
                    get_break().into(),
                    get_continue().into(),
                ],
                ..Default::default()
