...
impl CodeGenStream<'_> {
    ...
    fn generate_variant_common_field(&mut self, field_id: Ident) -> Result<(), std::io::Error> {
        writeln!(self, "int {field_id};")?;
        Ok(())
    }
    ...
    fn generate_variant_data(
        ...
        for (field_id, field_data) in fields.iter() {
            match field_data {
                VariantField::Common => self.generate_variant_common_field(*field_id)?,
